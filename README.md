...
┌─────────────────────────────────────────────────────────┐
│                  Frontend (Vue 3 + Vite)                 │
│  Nuxt UI · Pinia · Shiki · Mermaid · ApexCharts         │
└───────────────────────┬─────────────────────────────────┘
                        │ HTTP / SSE
┌───────────────────────┴─────────────────────────────────┐
│                  Spring Boot 4.1.0                       │
...
1. 打开浏览器访问 `http://localhost:3000`（开发模式）或 `http://localhost:8080`（`npm run build` 后由 Spring Boot 提供静态资源）
...
├── tests/                               # Playwright 测试
│   ├── frontend.spec.ts                 # 前端 DOM/SSE 测试
│   ├── integration-api.spec.ts          # API 集成测试
...
| UI 组件 | Nuxt UI v4（独立 Vue 模式） |
| 状态管理 | Pinia 2 |
| 流式对话 | @microsoft/fetch-event-source |
| Markdown | Shiki + markdown-it + Mermaid |
| 图表 | ApexCharts |
