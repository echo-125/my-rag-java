...
- Apache Tika 3.3.1（PDF/Word 文档解析）
...
1. 确保 PostgreSQL 运行，执行 `src/main/resources/db/init.sql`（创建数据库、pgvector 扩展、document_chunks 表）
2. 在 LLM 配置页面添加并激活至少一个模型
...
- PDF/Word/MD 使用 LangChain4j 的 DocumentByParagraphSplitter + Tika 解析。
...
- **无登录鉴权**：仅供本地单用户使用。
- **动态配置**：LLM 模型配置存储在数据库（llm_config 表），通过前端 LLM 配置页面管理。
- **SSE 实时推送**：数据入库和对话回复均通过 SSE 流式返回。

## 数据模型

### document_chunks 表（向量存储，手动创建）
...
- `file_path` — 源文件路径
- `language` — 编程语言
- `type` — 块类型（class/method/text 等）
- `signature` — 代码签名
- `start_line` / `end_line` — 源文件行号范围
- `created_at` — 创建时间

### llm_config 表（JPA 自动创建）

LLM API 配置：name、modelName、baseUrl、apiKey、apiFormat（openai_chat_completions/anthropic_messages）、isActive。
...
src/main/java/com/insolu/rag/
  InsoluRagApplication.java              启动类（含 LangChain4j HTTP 客户端配置）
  config/
    LangChain4jConfig.java               EmbeddingModel + PgVectorEmbeddingStore Bean
  controller/
    ...
    DashboardController.java             Dashboard 统计 + 最近问答
    ProjectConfigController.java         项目配置 CRUD
  entity/
    DocumentChunkEntity.java             向量存储只读 JPA 实体
    DocumentChunkRepository.java         统计查询（按语言/项目/文件）
  ...
  service/
    ...
    IngestionService.java                入库流程编排（批量 embedAll + SSE）
  ...
  splitter/
    ...
    FileSplitterRouter.java              按扩展名路由切分器 + Tika 文档解析

src/main/resources/
  application.yml                        Spring Boot + 数据库 + 日志配置
  db/init.sql                            数据库初始化脚本
  static/
    index.html                           单页面前端（4 标签页）
    lib/                                 echarts.min.js, mermaid.min.js, marked.min.js, tailwind.min.js
...
| POST | `/api/llm-configs/{id}/test` | 测试 API 连接 |
